        ...
        <div class="max-fit rounded overflow-hidden shadow-lg mx-auto sm:px-0 lg:px-0 bg-white">
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-0">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="grid grid-cols-1 gap-6 transition-colors py-6">
                            <div class="col-span-1">
                                <div class="card p-6 bg-gray-100 border-b border-black-200">
                                    <div class="card-body">
                                        ...
                                        <h3 class="card-title text-xl font-bold text-gray-800">{{ $coin->getName() }}</h3>
                                        ...
                                        @endif
                                        <button class="btn-primary bg-red-400 text-white rounded-full font-semibold px-12 py-12 hover:bg-red-600 focus:bg-red-600 focus:outline-none" style="margin-top: 20px" onclick="window.location.href='/crypto/{{$coin->getId()}}'">SELL</button>
                                        <button class="btn-primary bg-green-400 text-white rounded-full font-semibold px-12 py-12 hover:bg-green-600 focus:bg-green-600 focus:outline-none" style="margin-top: 20px" onclick="window.location.href='/crypto/{{$coin->getId()}}'">BUY</button>
                                        <p class="card-subtitle text-gray-600 p-lg-3">Market Cap: {{ number_format($coin->getMarketCap(),2) }} $</p>
                                        <p class="card-subtitle text-gray-600 p-lg-3">Volume 24h: {{ number_format($coin->getVolume24h(),2) }} $</p>
                                        <p class="card-subtitle text-gray-600 p-lg-3">Circulating Supply: {{ number_format($coin->getCirculatingSupply(),2) }} {{ $coin->getSymbol() }}</p>
                                        <p class="card-subtitle text-gray-600 p-lg-3">Total Supply: {{ number_format($coin->getTotalSupply(),2) }} {{ $coin->getSymbol() }}</p>
                                        <p class="card-subtitle text-gray-600 p-lg-3">Max Supply: {{ number_format($coin->getMaxSupply(),2) }} {{ $coin->getSymbol() }}</p>
                                        <p class="card-subtitle text-gray-600 p-lg-3">Last Updated: {{ $coin->getLastUpdated() }}</p>
                                    </div>
                                </div>
                            </div>
                    </div>
